...
- **AC-7c**: From the schedule detail modal, selecting additional connected accounts arms Save changes, shows a per-account datetime (default = original `scheduledAt`), and `POST /calendar/slots/:scheduleId/mirror` creates one new SocialMCP schedule per target account via `schedule_post` (same platform allowed when account ids differ). Each selected account has its own media (platform min/max caps); Instagram without media is rejected per Instagram target. New rows appear on the calendar week.
- **AC-7d**: Empty week-canvas hover shows a horizontal time guide and plus control at a snapped minute (5 min). Click opens an empty create schedule modal with `scheduledAt` from that guide (client IANA zone → UTC ISO). Past minutes and weeks with zero connected accounts do not open create (plus disabled or no-op). Slot cards still open the existing detail modal via `?schedule=`.
- **AC-7e**: The create modal lets the user select one or more connected accounts (same picker/carousel pattern as the detail modal). Each selected account has its own caption, datetime, and media (platform min/max caps). Save calls `POST /calendar/slots` with one or more targets and creates one SocialMCP `schedule_post` per account. Instagram without media is rejected per Instagram target; past `scheduledAt` is rejected. Create captions are per-account; detail-modal mirror caption remains shared.
- **AC-7f**: The create modal “Get ideas from Soc” strip calls `POST /calendar/compose-assist`. Empty or non-empty user message is allowed. Soc returns caption updates for the targeted account(s) (explicit platform mention in the message, else focused account, else all selected) and the UI writes those captions into the matching preview panes. Uses Thesean with business profile / tone context; soft-fails with `REWRITE_UNAVAILABLE`; never publishes or creates schedules by itself.
- **AC-7b**: `POST /calendar/slots/:scheduleId/rewrite-selection` accepts `{ selection, action: "regenerate"|"tweak"|"comment", instruction? }` and returns `{ suggestion }` from Thesean for selection-only caption rewrites. `POST /calendar/rewrite-selection` is the same Thesean helper without a schedule id (create board). `regenerate` needs no instruction; `tweak`/`comment` require an instruction of at most 40 words. Soft-fails with `REWRITE_UNAVAILABLE` when Thesean is missing or down; never auto-publishes.
...
| `/calendar/slots` | POST | Single `{ platform, accountId, scheduledAt, caption, media? }` **or** `{ targets: [{ platform, accountId, scheduledAt, caption, media? }], media? }` (per-target `media` preferred; shared body `media` is fallback) | `{ created: ScheduleDetail[] }` | session | 401, 422 past time / Instagram media / invalid account, 502 |
| `/calendar/compose-assist` | POST | `{ message?, targets: [{ accountId, platform, caption? }], focusAccountId? }` | `{ assistantText, updates: [{ accountId, caption }] }` | session | 401, 422, 503 Thesean unavailable |
...
| `/calendar/slots/:scheduleId/mirror` | POST | `{ targets: [{ platform, accountId, scheduledAt, media? }], caption?, media? }` (per-target `media` preferred; shared body `media` / source media is fallback) | `{ created: ScheduleDetail[] }` | session | 401, 404, 422, 409, 502 |
...
| Compose assist | caption updates + short reply | Thesean + profile/tone; soft-fail when unavailable; writes into create-board captions only |
...
- Calendar may create schedules via blank create (`POST /calendar/slots` → one or more SocialMCP `schedule_post` calls) and detail-modal mirror (`POST .../mirror`). Create-board captions are per-account; detail mirror caption stays shared. Chat / orchestration / planner remain alternate create paths.
